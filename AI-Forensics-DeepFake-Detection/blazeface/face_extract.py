...
class FaceExtractor:
    ...
    def process_image(self, path: str = None, img: Union[Image.Image, np.ndarray] = None) -> dict:
        """
        ...
        # Load and convert the image if a path is provided
        if img is None:
            try:
                img = np.asarray(Image.open(str(path)).convert("RGB"))
            except Exception as e:
                raise ValueError(f"Failed to load image from path: {path}. Error: {e}")
        else:
        ...
        # Add margin to detections and crop faces from the image
        frameref_detections = self._add_margin_to_detections(detections[0], frame_size, margin=0.2)
        faces = self._crop_faces(img, frameref_detections)

        # Crop keypoints with additional margin
        kpts = self._crop_kpts(img, detections[0], margin=0.3)
        ...
        # Sort faces by descending confidence scores
    ...
    def _tile_frames(self, frames: np.ndarray, target_size: Tuple[int, int]) -> Tuple[np.ndarray, List[float]]:
